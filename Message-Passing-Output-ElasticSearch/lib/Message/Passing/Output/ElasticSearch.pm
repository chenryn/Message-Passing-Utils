...
sub consume {
    ...
    my $type = delete($data->{'@type'}) || $data->{__CLASS__} || 'unknown';
    ...
    my $to_queue = {
        ...
        index => $index_name,
        data => {
            '@timestamp' => delete($data->{'@timestamp'}) || to_ISO8601DateTimeStr($date),
            '@tags' => delete($data->{'@tags'}) || [],
            '@type' => $type,
            '@source_host' => delete($data->{'@source_host'}) || 'none',
            '@message' => delete($data->{'@message'}) || encode_json($data),
            '@fields' => delete($data->{'@fields'}) || {},
        },
